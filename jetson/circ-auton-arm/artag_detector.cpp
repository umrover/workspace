...
pair<Tag, Tag> TagDetector::findARTags(Mat &src, Mat &depth_src, Mat &rgb) {  //detects AR tags in source Mat and outputs Tag objects for use in LCM
    ...
    if (ids.size() == 0) {
        ...
        discoveredTags.first.locM = Point2f();
    ...
    cerr << "two tags found" << endl;
        ...
        t0.locM = getTagCoordRelativeToCenter(corners[0], src, rgb, t0.loc);
        t1.id = ids[1];
        t1.loc = getAverageTagCoordinateFromCorners(corners[1]);
        t1.locM = getTagCoordRelativeToCenter(corners[1], src, rgb, t1.loc);
        ...
        t1.locM = getTagCoordRelativeToCenter(corners[ids.size() - 1], src, rgb,  t1.loc);
...
void TagDetector::updateDetectedTagInfo(rover_msgs::TargetPosition *arTags, pair<Tag, Tag> &tagPair, Mat &depth_img, Mat &src, Mat &rgb){
    ...
    tags.locxM.push_back(tagPair.first.locM.x);
